...
object Fibers extends IOApp.Simple {
  ...
  val sequentialOps = IO("Step1").debug >> IO("Step2").debug >> IO("Step3").debug
  ...
  val participant1WithFinaliser = participant1.onCancel(IO("Task 1 got canceled").debug.void)
  val participant2WithFinaliser = participant2.onCancel(IO("Task 2 got canceled").debug.void)
  ...
  val ioWithFallback = participant1.timeoutTo(1000.millis, IO("Fallback IO executed after timeout").debug)

  //override def run: IO[Unit] = ioWithFallback.void
  override def run: IO[Unit] = outcome.void
}
